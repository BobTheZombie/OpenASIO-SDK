#!/usr/bin/env bash
set -euo pipefail
PATCH="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/patches/openasio-driver-alsa17h.patch"

echo "== OpenASIO-SDK: Integrating ALSA Family 17h driver =="
[ -d .git ] || { echo "Run this script from your OpenASIO-SDK repo root (with .git)"; exit 1; }

# Ensure directories
mkdir -p crates/openasio-driver-alsa17h/src

# Try 3-way apply
set +e
git apply -3 --index "$PATCH"
RC=$?
set -e

if [ $RC -ne 0 ]; then
  echo "Patch didn't apply cleanly; writing files directly (fallback)."
  # Update workspace Cargo.toml: add member if missing
  if ! grep -q 'crates/openasio-driver-alsa17h' Cargo.toml; then
    awk '1; /^\[workspace\]/{f=1} f && /^\s*members\s*=\s*\[/{print; print "    \"crates/openasio-driver-alsa17h\","; f=0 }' Cargo.toml > Cargo.toml.tmp || true
    mv Cargo.toml.tmp Cargo.toml
  fi

  # Write crate files
  cat > crates/openasio-driver-alsa17h/Cargo.toml <<'EOF'
[package]
name = "openasio-driver-alsa17h"
version = "1.0.0"
edition = "2021"
license = "MIT OR Apache-2.0"
description = "OpenASIO driver for AMD Family 17h HDA-class ALSA controllers (full-duplex)"
categories = ["audio", "ffi"]
keywords = ["audio", "alsa", "hda", "openasio"]

[lib]
crate-type = ["cdylib"]

[dependencies]
openasio-sys = { path = "../openasio-sys" }
alsa = "0.9"
libc = "0.2"
...
EOF
  ...
  cat > crates/openasio-driver-alsa17h/src/lib.rs <<'EOF'
...
EOF
  ...
  git add -A
  git commit -m "feat(driver-alsa17h): add ALSA HDA Family 17h OpenASIO driver (full-duplex)"
else
  echo "Patch applied."
  git commit -m "feat(driver-alsa17h): add ALSA HDA Family 17h OpenASIO driver (full-duplex)" || true
fi

echo "Done. Build with: cargo build -p openasio-driver-alsa17h"
